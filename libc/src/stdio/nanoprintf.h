/* nanoprintf v0.5.5: a tiny embeddable printf replacement written in C.
   ...
   modified by asie (kontakt@asie.pl) for playing better with purpose-specific gcc-ia16 usage
   sourced from git commit 9c25f798f88302d768a016744d0ba1730f6548cc
...
// Wonderful: remove npf_ prefixes, add __far
#if defined(__IA16_CALLCVT_NO_ASSUME_SS_DATA)
# define NPF_STACK_ADDR_SPACE __far
# define NPF_STACK_ONLY_ADDR_SPACE __seg_ss
#else
# define NPF_STACK_ADDR_SPACE
# define NPF_STACK_ONLY_ADDR_SPACE
#endif

NPF_VISIBILITY int sprintf(
  char __far* NPF_RESTRICT buffer, const char __far* NPF_RESTRICT format, ...) NPF_PRINTF_ATTR(2, 3);

NPF_VISIBILITY int vsprintf(
  char __far* NPF_RESTRICT buffer, char const __far* NPF_RESTRICT format, va_list vlist) NPF_PRINTF_ATTR(2, 0);

NPF_VISIBILITY int snprintf(
  char __far* NPF_RESTRICT buffer, size_t bufsz, const char __far* NPF_RESTRICT format, ...) NPF_PRINTF_ATTR(3, 4);

NPF_VISIBILITY int vsnprintf(
  char __far* NPF_RESTRICT buffer, size_t bufsz, char const __far* NPF_RESTRICT format, va_list vlist) NPF_PRINTF_ATTR(3, 0);

typedef void (*npf_putc)(int c, void NPF_STACK_ADDR_SPACE* ctx);
NPF_VISIBILITY int npf_pprintf(npf_putc pc,
                               void NPF_STACK_ADDR_SPACE* NPF_RESTRICT pc_ctx,
                               char const __far* NPF_RESTRICT format,
                               ...) NPF_PRINTF_ATTR(3, 4);
...
static int npf_parse_format_spec(char const __far* format, npf_format_spec_t NPF_STACK_ONLY_ADDR_SPACE* out_spec) {
...
static NPF_NOINLINE int npf_utoa_rev(
    npf_uint_t val, char NPF_STACK_ONLY_ADDR_SPACE* buf, uint_fast8_t base, char case_adj) {
  uint_fast8_t n = 0;
  do {
...
static int npf_ftoa_rev(char __far* buf, npf_format_spec_t const *spec, double f) {
  char const* ret = NULL;
...
static void npf_bufputc(int c, void NPF_STACK_ADDR_SPACE* ctx) {
  npf_bufputc_ctx_t NPF_STACK_ADDR_SPACE* bpc = (npf_bufputc_ctx_t NPF_STACK_ADDR_SPACE*) ctx;
  if (bpc->cur < bpc->len) { bpc->dst[bpc->cur++] = (char)c; }
}

static void npf_bufputc_nop(int c, void NPF_STACK_ADDR_SPACE* ctx) { (void)c; (void)ctx; }

typedef struct npf_cnt_putc_ctx {
  npf_putc pc;
  void NPF_STACK_ADDR_SPACE* ctx;
  int n;
} npf_cnt_putc_ctx_t;

static void npf_putc_cnt(int c, void NPF_STACK_ADDR_SPACE* ctx) {
  npf_cnt_putc_ctx_t NPF_STACK_ADDR_SPACE* pc_cnt = (npf_cnt_putc_ctx_t NPF_STACK_ADDR_SPACE*) ctx;
  ++pc_cnt->n;
...
int npf_vpprintf(npf_putc pc, void NPF_STACK_ADDR_SPACE* pc_ctx, char const __far* format, va_list args) {
  ...
  if ((fs.prec_opt == NPF_FMT_SPEC_OPT_NONE) &&
    ...
    switch (fs.conv_spec) {
      ...
      case NPF_FMT_SPEC_CONV_CHAR:
        *u.cbuf_mem = (char)va_arg(args, int);
        cbuf_len = (*u.cbuf_mem) ? 1 : 0;
        break;
      ...
      case NPF_FMT_SPEC_CONV_STRING: {
        ...
        for (char const __far* s = cbuf;
             ...
             ++s, ++cbuf_len);
#else
        for (char const __far* s = cbuf; cbuf && *s; ++s, ++cbuf_len); // strlen
#endif
      } break;
      ...
      case NPF_FMT_SPEC_CONV_SIGNED_INT: {
...
#endif
        {
          ...
          cbuf_len = npf_utoa_rev(uval, u.cbuf_mem, 10, fs.case_adjust);
        }
      } break;
      ...
      case NPF_FMT_SPEC_CONV_POINTER: {
...
#endif
        {
          ...
          cbuf_len = npf_utoa_rev(val, u.cbuf_mem, base, fs.case_adjust);
        }

#if NANOPRINTF_USE_ALT_FORM_FLAG == 1
        if (val && fs.alt_form && (fs.conv_spec == NPF_FMT_SPEC_CONV_OCTAL)) {
          u.cbuf_mem[cbuf_len++] = '0'; // OK to add leading octal '0' immediately.
        }
...
int sprintf(char __far* buffer, const char __far* format, ...) {
  va_list val;
  va_start(val, format);
  int const rv = vsnprintf(buffer, SIZE_MAX, format, val);
  va_end(val);
  return rv;
}

int vsprintf(char __far* buffer, const char __far* format, va_list vlist) {
  return vsnprintf(buffer, SIZE_MAX, format, vlist);
}

int snprintf(char __far* buffer, size_t bufsz, const char __far* format, ...) {
  va_list val;
  va_start(val, format);
...
int vsnprintf(char __far* buffer, size_t bufsz, char const __far* format, va_list vlist) {
